...
class DatasetBuilder:
    ...
    def generate_distilled_kokoro_corpus(
        self,
        quote_list: Optional[List[str]] = None,
        cfg_strength: float = 5.5,
        nfe_step: int = 48,
        ...
        """
        Synthesizes an expanded, highly expressive speech corpus using F5-TTS teacher
        at native 24kHz and merges it into Kokoro's dataset (datasets/kokoro/wavs).
        """
        f5_ref_wav = self.datasets_dir / "f5tts" / "ref_audio" / "ref.wav"
        f5_ref_txt = self.datasets_dir / "f5tts" / "ref_audio" / "ref.txt"
        ref_text_val = ""
        ...
        wavs_dir = kokoro_dir / "wavs"
        wavs_dir.mkdir(parents=True, exist_ok=True)
        metadata_csv = kokoro_dir / "metadata.csv"
        ...
        for idx, text in enumerate(quotes):
            ...
            if progress_callback:
                pct = ((idx + 1) / total_quotes) * 100.0
                progress_callback(pct, f"Distilling Kokoro 24kHz clip {idx + 1}/{total_quotes}: {text[:35]}...")

            clean_text = text.strip().replace("|", " ")

            # F5-TTS outputs native 24kHz audio matching Kokoro's 24kHz sample rate directly
            f5_model.infer(
                ref_file=str(f5_ref_wav.resolve()),
                ref_text=ref_text_val,
                gen_text=clean_text,
                file_wave=str(target_wav),
                speed=float(speed),
                seed=42 + idx,
                cfg_strength=float(cfg_strength),
                nfe_step=int(nfe_step)
